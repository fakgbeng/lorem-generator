...
const numberofPara = document.querySelector(".lorem-form input");
const generateBtn = document.querySelector(".btn");
const loremPara = document.querySelector(".result");
// console.log(loremPara);

generateBtn.addEventListener("click", function (e) {
  e.preventDefault();
  loremPara.innerHTML = "";
  const number = +numberofPara.value;
  if (number <= text.length && number > 0) {
    for (let i = 0; i < number; i++) {
      if (text[i] === undefined) return;
      const html = `<p>${text[i]}</p>`;
      loremPara.insertAdjacentHTML("beforeend", html);
    }
  } else return (loremPara.innerHTML = `<p>${text[0]}</p>`);
});
